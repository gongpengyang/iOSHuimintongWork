...
#ifndef kIsNetwork
#define kIsNetwork     [HKNetwork isNetwork]  // 一次性判断是否有网的宏
#endif

#ifndef kIsWWANNetwork
#define kIsWWANNetwork [HKNetwork isWWANNetwork]  // 一次性判断是否为手机网络的宏
#endif

#ifndef kIsWiFiNetwork
#define kIsWiFiNetwork [HKNetwork isWiFiNetwork]  // 一次性判断是否为WiFi网络的宏
#endif
 ...
 **/
/**缓存策略*/
typedef NS_ENUM(NSUInteger, HKCachePolicy){
    /**只从网络获取数据，且数据不会缓存在本地**/
    HKCachePolicyOnlyNetNoCache = 0,
    /** 从缓存读取数据并返回，再从网络获取并缓存，每次只读取缓存数据 这种一般用于广告页面加载模式**/
    HKCachePolicyCacheElseNet,
    /** 先从网络获取数据并缓存数据，如果访问网络失败再从缓存读取，失败的Block和成功的Block都会执行*/
    HKCachePolicyNetElseCache,
    /**先从缓存读取数据，然后在从网络获取并且缓存，在这种情况下，缓存数据与网络数据不一致Block将产生两次调用*/
    HKCachePolicyCacheThenNet
};

/**请求方式*/
typedef NS_ENUM(NSUInteger, HKRequestMethod){
    /**GET请求方式*/
    HKRequestMethodGET = 0,
    /**POST请求方式*/
    HKRequestMethodPOST,
    /**HEAD请求方式*/
    HKRequestMethodHEAD,
    ...
    HKRequestMethodPUT,
    /**PATCH请求方式*/
    HKRequestMethodPATCH,
    /**DELETE请求方式*/
    HKRequestMethodDELETE
};

typedef NS_ENUM(NSUInteger, HKNetworkStatusType){
    /**未知网络*/
    HKNetworkStatusUnknown,
    /**无网路*/
    HKNetworkStatusNotReachable,
    /**手机网络*/
    HKNetworkStatusReachableWWAN,
    /**WiFi网络*/
    HKNetworkStatusReachableWiFi,
    
};

typedef NS_ENUM(NSUInteger, HKRequestSerializer){
    /**设置请求数据为JSON格式*/
    HKRequestSerializerJSON,
    /**设置请求数据为二进制格式*/
    HKRequestSerializerHTTP
};

typedef NS_ENUM(NSUInteger, HKResponseSerializer) {
    /**设置响应数据为JSON格式*/
    HKResponsetSerializerJSON,
    /**设置响应数据为二进制格式*/
    HKResponseSerializerHTTP
};

/**请求的成功Block*/
typedef void(^HKHttpSuccess)(id responseObject);

/**请求的失败Block*/
typedef void(^HKHttpFail)(NSError *error);

/**下载的Block*/
typedef void(^HKHttpDownload)(NSString *path);

/**上传或者下载的进度**/
typedef void(^HKHttpProgress)(NSProgress *progress);

/**网络状态Block*/
typedef void(^HKNetworkStatus)(HKNetworkStatusType status);

@interface HKNetwork : NSObject
...
/**
 ...
 @param requestSerializer HKRequestSerializerJSON---JSON格式  HKRequestSerializerHTTP--HTTP
 */
...
/**
 ...
 @param responseSerializer HKResponseSerializerJSON---JSON格式  HKResponseSerializerHTTP--HTTP
